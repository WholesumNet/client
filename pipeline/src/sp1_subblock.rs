...
#[derive(Debug, Clone)]
pub enum Input {    
    // a blob, used in initial and final rounds
    Blob(Vec<u8>),
...
pub struct Pipeline {
    ...
    // execute_aggregation_round: Round,    
}
...
impl Pipeline {
    pub fn new() -> anyhow::Result<Self> {        
        Ok(Self {
            ...
            // execute_aggregation_round: Round::new(788, 1),
        })
    }
    
    pub fn feed_subblock_stdin(&mut self, index: usize, blob: Vec<u8>) {
        self.execute_subblock_round.feed_input(index, Input::Blob(blob));
    }
